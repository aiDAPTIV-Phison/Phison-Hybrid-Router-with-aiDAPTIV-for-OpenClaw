...
- A2UI bundle hash: `src/canvas-host/a2ui/.bundle.hash` is auto-generated; ignore unexpected changes, and only regenerate via `pnpm canvas:a2ui:bundle` (runs `scripts/bundle-a2ui.mjs`; `scripts/bundle-a2ui.sh` for manual bash runs) when needed. Commit the hash as a separate commit.
